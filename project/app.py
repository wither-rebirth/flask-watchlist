...
def test_url_for():
    ...
    print(url_for('hello')) #生成hello视图函数对应的URL，将会输出: /
    #注意下面两个调用是如何生成包含URL变量的URL的
    print(url_for('user_page', name='geryli')) #输出 /user/geryli
...
class User(db.Model, UserMixin):
    ...
    def set_password(self, password): #用来设置密码的方法，接受密码作为参数
        self.password_hash = generate_password_hash(password) #将生成的密码放置到对应字段
...
class Movie(db.Model): #表名将会是 movie
    id = db.Column(db.Integer, primary_key=True) #主题
    title = db.Column(db.String(60)) #电影标题
    year = db.Column(db.String(4)) #电影年份


@app.cli.command() #注册为命令，可以传入name 参数来自定义命令
...
def initdb(drop):
    ...
    if drop:#判断是否输入选项
        db.drop_all()
    db.create_all()
    click.echo('Initialized database.') #输出提示消息
